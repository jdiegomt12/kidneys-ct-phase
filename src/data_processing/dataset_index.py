...
def _get_cache_path(root: Path) -> Path:
    ...
    return root.parent / ".index_cache" / f"{root.name}.pkl"


def _compute_dir_hash(root: Path) -> str:
    """Compute hash of directory structure to detect changes."""
    try:
        case_dirs = sorted([p for p in root.iterdir() if is_numeric(p)], key=lambda x: int(x.name))
        dir_info = ";".join([f"{d.name}:{os.path.getmtime(d)}" for d in case_dirs if d.exists()])
        return hashlib.md5(dir_info.encode()).hexdigest()
    except:
        return ""


def load_cached_candidates(root: Path) -> Optional[Dict]:
    """Load cached candidates if structure hasn't changed."""
    ...
    try:
        with open(cache_path, 'rb') as f:
            ...
            if cached.get("dir_hash") == _compute_dir_hash(root):
...
def save_candidates_cache(root: Path, candidates_per_case: Dict):
    """Save candidates to persistent cache."""
    cache_path = _get_cache_path(root)
    cache_path.parent.mkdir(parents=True, exist_ok=True)
    
    try:
        cache_data = {
            "dir_hash": _compute_dir_hash(root),
...
def split_mixed_series(dicom_dir: Path, headers_cache: Optional[Dict] = None) -> List[Dict[str, str]]:
    """
    ...
    Args:
        ...
        headers_cache: Cached DICOM headers to avoid re-reading
    """
    ...
    for fname in files:
        ...
        # Check cache first
        if filepath in headers_cache:
            series_uid = headers_cache[filepath].get("series_uid", "")
            study_uid = headers_cache[filepath].get("study_uid", "")
        else:
            # Read from DICOM
            try:
                reader = sitk.ImageFileReader()
                ...
                headers_cache[filepath] = {"series_uid": series_uid, "study_uid": study_uid}
            except Exception:
                continue
...
def build_index(
    ...
    *,
    ...
    # Try to load cached candidates to skip already-processed cases
    cached_candidates = load_cached_candidates(root)
    all_candidates = {}
    headers_cache = {}  # Shared cache for DICOM headers
    ...
    for idx, case_dir in enumerate(case_dirs, 1):
        ...
        # Check cache first
        if cached_candidates and case_id in cached_candidates:
            # Reconstruct candidates from cache (skip expensive scanning)
            candidates = cached_candidates[case_id]
        else:
            candidates = series_candidates(search_root)
            all_candidates[case_id] = candidates
        ...
        for cand in candidates:
            if cand.matched_phase in ("arterial", "late") and cand.n_files > 350:
                ...
                if is_mixed:
                    mixed_series_cache[str(cand.dicom_dir)] = split_mixed_series(cand.dicom_dir, headers_cache)
    ...
    # Save candidates cache for next run
    if all_candidates:
        save_candidates_cache(root, all_candidates)
